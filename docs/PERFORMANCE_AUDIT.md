...
| --- | --- | --- |
| Inventory defaults: exact stock-list record, then availability, hire-stock or consumables fallbacks | `N` lookups started without a global limit; each lookup could use 1–4 requests. | Still required for missing master data, but one automatic request chain at a time, at least 1.25 seconds between chains, 0.5 seconds between fallbacks, identical in-flight deduplication, a 15-minute session cache only for successful commercial-field results, short retry for empty results, paused while hidden, and server-directed/global cooldown after rate limiting. |
| Proposal preview: `/modules/docmaker/merge-html.php`, QTC V4 document 169 | One render when opened; debounced renders after relevant item mutations. | Unchanged because it is required functionality. In-flight refresh coalescing remains. Default opening is deferred while the browser tab is hidden. |
