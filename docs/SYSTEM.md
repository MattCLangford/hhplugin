...
`8-stagedesigner.js` adds a simple staging kit designer to the supplying list toolbar. It loads live HireHop items from the Staging category via the availability product list, imports the generated stage rows under a stage heading using `items_import.php`, and creates custom placeholder rows only for consumables until carpet and fascia stock IDs are available.
