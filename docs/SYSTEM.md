...
It also owns the shared automatic-read queue. HireHop reads are prioritised, deduplicated, cached where safe, and serialised with a conservative minimum gap. A detected rate-limit response pauses queued work instead of causing a retry burst. Loader and request diagnostics are available through `WiseHireHopDiagnostics.describe()`. See [PERFORMANCE_AUDIT.md](PERFORMANCE_AUDIT.md) for the request inventory and live verification checklist.
