...
`1-docprev.js` is essential because it lets users see what their list edits do to the rendered proposal. On an eligible supplying-list page it opens docked by default as soon as its toolbar entry point is mounted. The user can close it normally, and a one-time startup guard prevents later refreshes or DOM activity from forcing it open again during that page session. At the bottom of the supplying list, the module also mounts a compact Job Performance strip. A hidden same-origin Job Track render (`doc=162`) remains the source of truth, so its final post-assumption Revenue, COS, GP£, GP%, and Review/OK result stay aligned with the full document rather than duplicating the costing model in UI code. The strip translates that result into Strong, On track, or Needs review, with threshold and target context, and refreshes after supplying-list saves. The native footer containing Replacement cost, Weight, Volume, Discount/Markup, and Quoted net total is hidden presentation-only using its unique labels; no HireHop value is changed or deleted. A replacement Commercial adjustments row shows the job's Discretionary discount, Venue commission, and Client commission custom fields from the same live source. Power users can therefore work directly in HireHop's native list editing with both the proposal preview and commercial health immediately visible.
