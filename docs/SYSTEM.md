...
`7-captrack.js` is the Wise capacity tracker. Its home-page entry button is visible across all depots. It opens as a full-page planning view and uses project-level Wise custom fields for event naming, status, revenue, tier, client, venue, and role allocation. Technical and Production can be viewed separately or together; these manager views show subtle absence-style `DaysPrior` and `DaysPost` allocation buffers whose outer handles save whole-day changes directly to the project custom fields. Buffer saves route only through HireHop's native project record ID (never the Wise ID), post that value as the endpoint's required `id` update parameter, and post the partial custom-field bag in HireHop's JSON format. A fail-closed preflight blocks the request entirely unless the ID is a positive integer and exactly one valid buffer field is present, preventing the endpoint's missing-ID create behaviour.

`8-stagedesigner.js` is retained as a rollback implementation but is globally disabled in `0-loader.js`, so it is not requested in any depot. If restored, it adds a simple staging kit designer to the supplying list toolbar and loads live HireHop items from the metric Staging category, the imperial Unit 10 Stock staging family, and the Unit 10 Consumables sales-stock category for imperial carpet/felt. It imports generated stage rows with `items_import.php`; its original depot safeguards and generated-row behaviour remain in source.

`16-externalmod.js` loads the pinned third-party Stage Designer loader in every depot, independently of the shared depot detector. The remote loader owns its normal HireHop New/context-menu integration; the bridge retains an API `1.x`-compatible fallback adapter, URL validation, duplicate-load protection, diagnostics and failure isolation.
