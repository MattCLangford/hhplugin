...
`12-projectgroups.js` is Proposal Creation depot only, gated on the logged-in user's active depot (a session-level permissions barrier, not which depot the project itself belongs to). It reads `window.user`'s own depot field directly rather than the shared `5-hirehop.js` depot-detection helper — that helper's page-wide `select,input,textarea` scan was confirmed (via its own `debug()` output) to always resolve to "Proposal Creation" on the project details page regardless of the user's real active depot, because it was matching an unrelated always-present element rather than the header depot switcher. It groups the native `#proj_info` project fields into 5 logical groups — System Details, Wise Project Details, Project Ownership, Operational Timings, Working Links — by wrapping the elements that already sit together in the DOM with `.wrapAll()`. The wrapper uses `display:contents` so it never changes layout: `#proj_info`'s CSS Grid and `#custom_fields_container`'s flex-wrap keep placing every field exactly where it renders today. It then swaps the two top-level blocks (a plain DOM move) so the Salesforce/Wise group (`#custom_fields_container`) renders before the native System Details block, so Salesforce/event info leads and native HireHop record fields take a back seat. No new styling, headings, relabeling, or colour is applied yet — that is a deliberate follow-up pass. Every other depot leaves this module inactive and the native screen fully untouched.
