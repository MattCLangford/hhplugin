...
`13-proposalpageicons.js` is a Proposal Creation depot-only supplying-list visual guide. It replaces native folder icons for `Section:` and `Dept:` headings with a compact cream/gold document-page icon styled to sit alongside HireHop's native tree icons, while `// Section:` and `// Dept:` headings receive a muted grey slashed-page icon. It reads the logged-in user's active depot from `window.user`, fails closed when that depot cannot be confirmed, and only changes rendered icon classes—the supplying-list names, records, hierarchy, and unmatched native icons are not modified.
