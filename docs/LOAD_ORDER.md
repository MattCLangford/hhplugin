...
`15-supplyingcommercial.js` loads with the supplying-list bundle and activates only for Proposal Creation. It adds editable line-level `Revenue` and `Markup` custom fields to HireHop's native inventory-item dialog, including HireHop's namespaced supplying-row fields such as `items:_Revenue` and `items:_Markup`. Entering Markup calculates Revenue from the native line Total (shown as CoS); entering Revenue calculates and saves a whole-number Markup. The grid is presented as CoS, Markup, Revenue. Native Unit Price is hidden presentation-only; the underlying Unit Price, Discount/Markup, Flag and Total values remain intact.
