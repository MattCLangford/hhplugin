...
From `15-supplyingcommercial.js` cache version `2.3`, the native-popup integration described above is retired. From cache version `2.4`, each inventory row instead has a compact pen action beside its value in the Revenue column. It opens a standalone Markup/Revenue editor bound directly to that supplying line and performs a partial `items_save.php` update containing only the line identity, kind and merged `custom_fields`; it does not resubmit Qty, dates, pricing, memo, parent or other native item values. The action shares the Revenue cell rather than creating a separate heading or table column.
