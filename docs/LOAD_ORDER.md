...
`15-supplyingcommercial.js` loads with the supplying-list bundle and activates only for Proposal Creation. It adds editable line-level `Revenue` and `Markup` custom fields to HireHop's native inventory-item dialog, including HireHop's namespaced supplying-row fields such as `items:_Revenue` and `items:_Markup`. Every popup is bound to its selected supplying-row node and reads that live line state; inventory-master custom fields are deliberately excluded. Entering Markup calculates Revenue from the native line Total (shown as CoS); entering Revenue calculates and saves a whole-number Markup. Changing native Qty retains Markup and recalculates Revenue after HireHop updates the line CoS. The grid is presented as CoS, Markup, Revenue. Native Unit Price is hidden presentation-only; the underlying Unit Price, Discount/Markup, Flag and Total values remain intact.
