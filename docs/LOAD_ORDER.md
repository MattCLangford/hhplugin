...
On the supplying list, `1-docprev.js` opens the docked proposal preview by default once its toolbar button is ready. The user can close it normally; subsequent page activity does not force it open again during that page session. Directly below the native button toolbar and above the list, the same module mounts the Commercial adjustments reminder followed by the lightweight Job Performance strip. Revenue is the sum of the supplying-line `Revenue` custom fields, CoS is the sum of the native line `Total` values, GP£ is Revenue minus CoS, and GP% is GP£ divided by Revenue. The live Job Track render (`doc=162`) supplies only the three adjustment percentages and its flag for reference. The strip's solid-colour totaliser shows progress from 0% to the required 45% GP target, while the fill colour reflects the supplying-line GP% on an accelerated below-target dark-blue curve, then green at 45%, dark green at 65%, amber at 75%, red at 90%, and deep red at 100%. The native totals footer remains hidden presentation-only.
...
`15-supplyingcommercial.js` loads with the supplying-list bundle and activates only for the logged-in user's active Proposal Creation depot. It adds editable line-level `Revenue` and `Markup` custom fields to HireHop's native inventory-item dialog, including HireHop's namespaced supplying-row fields such as `items:_Revenue` and `items:_Markup`. Every popup is bound to its selected supplying-row node and reads that live line state; inventory-master custom fields are deliberately excluded. Entering Markup calculates Revenue from the native line Total (shown as CoS); entering Revenue calculates and saves a whole-number Markup. A live watcher follows HireHop's visible CoS control, so changing native Qty retains Markup and recalculates Revenue even when HireHop updates CoS without emitting a form event. The supplying grid is presented as CoS, Markup, Revenue using either the live jsTree API metadata or the rendered native grid columns, so the layout remains available when HireHop does not expose the tree API instance. Native Unit Price is hidden presentation-only; the underlying Unit Price, Discount/Markup, Flag and Total values remain intact.
