...
On the supplying list, `1-docprev.js` opens the docked proposal preview by default once its toolbar button is ready. The user can close it normally; subsequent page activity does not force it open again during that page session. Directly below the native button toolbar and above the list, the same module mounts the Commercial adjustments reminder followed by the lightweight Job Performance strip. Base Revenue is the sum of each supplying line's explicit `Revenue` value or its inventory-master default when the line field is absent, while base CoS is the sum of native line `Total` values. The live Job Track render (`doc=162`) supplies the three commercial adjustment percentages and its flag: discretionary discount reduces Revenue first, then venue and client commission are calculated on discounted Revenue and added to CoS. GP£ and GP% are derived from those post-adjustment totals. The strip's solid-colour totaliser shows progress from 0% to the required 45% GP target, while the fill colour reflects the post-adjustment GP% on an accelerated below-target dark-blue curve, then green at 45%, dark green at 65%, amber at 75%, red at 90%, and deep red at 100%. The native totals footer remains hidden presentation-only.
