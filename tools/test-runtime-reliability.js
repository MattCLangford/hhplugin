...
function testSourceGuards() {
  ...
  assert(externalMod.includes("HH-YES-Plugins@v0.1.111/loader-stage-designer.js"), "the bridge should retain the reviewed pinned Stage Designer loader");
  ...
  assert(jobGroups.includes('var PROJECT_GROUPS = ['), "job details should render a dedicated parent-project overview outside the job-owned cards");
  ...
  assert(jobGroups.includes('wise-jg-project-date-grid') && jobGroups.includes('wise-jg-project-info-grid'), "project timings and project information should have independent compact grids");
  assert(jobGroups.includes(".wise-jg-project-date-grid{display:grid;grid-template-columns:repeat(9,minmax(0,1fr))"), "project timing cards should use the same nine-column sizing basis as job timing cards even when only six are present");
  ...
  assert(jobGroups.includes("[data-wise-job-group='job-timings']{grid-column:1 / -1;grid-row:2;}"), "job timings should sit full width below the two job information cards");
  assert(jobGroups.includes("$layout.append($jobGrid, $projectGrid)"), "the read-only project window should follow all job-owned sections");
  assert(jobGroups.includes("Project-level view · read only here") && jobGroups.includes("--wise-project-window-accent"), "the parent-project window should clearly identify its read-only scope and use an independent project accent");
  assert(jobGroups.includes("height:74px;min-height:74px;max-height:74px"), "all timing cards should keep the same compact fixed height regardless of label length");
  ...
  assert(!jobGroups.includes("[data-wise-job-group='project-details']{border-left-width:10px;}"), "the project overview should avoid competing with the stronger job-status accent rail");
